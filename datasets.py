...
class NextTokenPredictionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        sequence = self.all_tokens[idx*self.context_len: (idx+1)*self.context_len]
        label = self.all_tokens[idx*self.context_len+1: (idx+1)*self.context_len+1]
...
class LitNextTokenPredictionDataModule(L.LightningDataModule):
    def __init__(
        ...
        args:
            ...
            context_len: max sequence length
        '''
        assert isinstance(train_data, (str, Dataset)), \
        'train_data should be a str or a torch Dataset'
        assert tokenizer_path is not None, \
        f'must provide tokenizer_path'
        if val_data:
            assert isinstance(val_data, (str, Dataset)), \
            'val_data should be a str or a torch Dataset'
        if test_data:
            assert isinstance(test_data, (str, Dataset)), \
