...
class Wrapper(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        cosine_lr = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max_epochs)
    ...
    def training_step(self, batch):
        ...
        if self.trainer.is_last_batch:  # and (self.trainer.current_epoch+1)%10 == 0:
            lr_sch.step()
        log = {f"train_loss": loss.item(), "train_acc": acc, "train_ppl": ppl}
    ...
    def validation_step(self, batch):
        ...
        acc = self.model.accuracy(label, logit)
        ppl = perplexity(logit, label, ignore_index=-1).item()
        log = {f"val_loss": loss.item(), "val_acc": acc, "val_ppl": ppl}
        self.log_dict(log, prog_bar=True, on_step=True, on_epoch=True)
        # return loss
    ...
    def test_step(self, batch):
        ...
        acc = self.model.accuracy(label, logit)
        ppl = perplexity(logit, label, ignore_index=-1).item()
        log = {f"test_loss": loss.item(), "test_acc": acc, "test_ppl": ppl}
        self.log_dict(log, prog_bar=True, on_step=True, on_epoch=True)
        # return loss
