...
def plot_metrics_v2(metrics: dict[str, str], save_dir=None):
    ...
    # print(save_dir)
    df_metrics = {k: pd.read_csv(v) for k, v in metrics.items()}
    ...
    # print(epoch_metrics)
    # print(step_metrics)
    df_metrics = {k: v[index_columns + metric_names] for k, v in df_metrics.items()}
    # print(df_metrics['abs'][:5])
    df_metrics = {
        k: v.rename(columns=lambda c: f"{k}_{c}" if c not in index_columns else c)
