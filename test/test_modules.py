...
class TestModules(unittest.TestCase):
    ...
    def test_RotaryEmbedding(self):
        ...
        q = torch.randn(self.batch_size, self.seq_len, self.n_heads, self.head_dims)
        k = torch.randn(self.batch_size, self.seq_len, self.kv_heads, self.head_dims)
        rope = RotaryEmbedding(
            dim=self.head_dims, seq_before_head_dim=True, use_xpos=True
        )
    ...
    def test_SinusoidalPositionalEncoding(self):
        ...
        q = torch.randn(self.batch_size, self.seq_len, self.d_query)
        k = torch.randn(self.batch_size, self.seq_len, self.d_key)
        spe = SinusoidalPositionalEncoding(d_query=self.d_query, d_key=self.d_key)
    ...
    def test_MultiheadGQA_v2(self):
        ...
        abs_config = dict(
            d_query=self.embed_dim, d_key=kv_embed_dim, dropout=0.1, max_len=1024
        )
